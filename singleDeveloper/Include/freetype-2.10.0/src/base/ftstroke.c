...
FT_CALLBACK_TABLE const FT_Glyph_Class
...
if( stroker )
{
ft_stroke_border_reset( &stroker
...
FT_Vector_From_Polar( &delta2, radius, angle
...
delta.x += stroker->center.x + delta2.

x;

delta.y += stroker->center.y + delta2.
...
delta.x += delta2.x + stroker->center.

x;

delta.y += delta2.y + stroker->center.
...
delta.x += stroker->center.

x;

delta.y += stroker->center.
...
if( !intersect )
{
FT_Vector_From_Polar( &delta, stroker
...
middle.x += stroker->center.

x;

middle.y += stroker->center.
...
static FT_Error
  ft_stroker_process_corner( FT_Stroker
...
static FT_Error
  ft_stroker_subpath_start( FT_Stroker
...
x = stroker->center.x + delta.x;

point.

y = stroker->center.y + delta.y;
...
subpath_line_length = line_length;
...
ft_pos_abs( FT_Angle_Diff( stroker->angle_in, angle_in )
...
line_join = stroker->line_join_saved;
}
...
FT_Vector_From_Polar( &ctrl, length, phi
...
/* compute end point */
FT_Vector_From_Polar( &end, stroker

->radius, angle_out + rotate );
end.x += arc[0].
...
ft_pos_abs( FT_Angle_Diff( alpha0, alpha1 )
...
FT_Vector_From_Polar( &ctrl1, length1, phi1
...
FT_Vector_From_Polar( &ctrl2, length2, phi2
...
static FT_Error
  ft_stroker_add_reverse_left( FT_Stroker
...
num_points = ( FT_UInt ) left->start;

right->num_points += (FT_UInt)
...
angle_out = stroker->subpath_angle;
...
ft_stroke_border_close( stroker
...
FT_Stroker_ExportBorder( FT_Stroker
...
FT_CURVE_TAG( outline

->tags[last] ) == FT_CURVE_TAG_ON )
{
...
x = ( v_control.x + vec.x ) / 2;

v_middle.

y = ( v_control.y + vec.y ) / 2;
