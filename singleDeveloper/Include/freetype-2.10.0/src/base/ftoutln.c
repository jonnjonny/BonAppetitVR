...
n_points = ( FT_Short ) numPoints;

anoutline->

n_contours = ( FT_Short ) numContours;
...
x = FT_MulDiv( shift.x, xstrength, d );

else
shift.

x = FT_MulDiv( shift.x, l, q );


if(

FT_MulFix( ystrength, q
...
y = FT_MulDiv( shift.y, ystrength, d );

else
shift.

y = FT_MulDiv( shift.y, l, q );
}
else
shift.
...
{
points[i].x += xstrength + shift.

x;

points[i].y += ystrength + shift.
...
x = points[last].x >> xshift;

v_prev.

y = points[last].y >> yshift;
