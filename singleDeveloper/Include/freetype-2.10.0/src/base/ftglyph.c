...
xMax = cbox->xMin + ( FT_Pos )( glyph->bitmap.width * 64 );
...
yMin = cbox->yMax - ( FT_Pos )( glyph->bitmap.rows * 64 );
}
...
ft_outline_glyph_init( FT_Glyph
...
ft_outline_glyph_done( FT_Glyph
...
FT_Outline_Done( FT_GLYPH( glyph )

->library, &glyph->outline );
}


FT_CALLBACK_DEF( FT_Error )

ft_outline_glyph_copy( FT_Glyph
...
ft_outline_glyph_transform( FT_Glyph
...
ft_outline_glyph_bbox( FT_Glyph
...
ft_outline_glyph_prepare( FT_Glyph
...
ft_new_glyph( library, clazz, aglyph
...
xMin = acbox->yMin = acbox->xMax = acbox->yMax = 0;
...
xMin = FT_PIX_FLOOR( acbox->xMin );

acbox->

yMin = FT_PIX_FLOOR( acbox->yMin );

acbox->

xMax = FT_PIX_CEIL_LONG( acbox->xMax );

acbox->

yMax = FT_PIX_CEIL_LONG( acbox->yMax );
}
...
error = ft_bitmap_glyph_init( ( FT_Glyph )
