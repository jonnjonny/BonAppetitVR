...
ft_service_list_lookup( FT_ServiceDesc
...
ft_validator_init( FT_Validator
...
ft_validator_error( FT_Validator
...
memory = memory;  /* just to be certain */
...
ft_glyphslot_free_bitmap( FT_GlyphSlot
...
ft_glyphslot_preset_bitmap( FT_GlyphSlot
...
xMin = ( cbox.xMin >> 6 ) + ( x_shift >> 6 );

pbox.

yMin = ( cbox.yMin >> 6 ) + ( y_shift >> 6 );

pbox.

xMax = ( cbox.xMax >> 6 ) + ( x_shift >> 6 );

pbox.

yMax = ( cbox.yMax >> 6 ) + ( y_shift >> 6 );

/* tiny remainder box */
cbox.

xMin = ( cbox.xMin & 63 ) + ( x_shift & 63 );

cbox.

yMin = ( cbox.yMin & 63 ) + ( y_shift & 63 );

cbox.

xMax = ( cbox.xMax & 63 ) + ( x_shift & 63 );

cbox.

yMax = ( cbox.yMax & 63 ) + ( y_shift & 63 );
...
pixel_mode = ( unsigned char ) pixel_mode;
...
ft_glyphslot_set_bitmap( FT_GlyphSlot
...
ft_glyphslot_alloc_bitmap( FT_GlyphSlot
...
static void
  ft_glyphslot_clear( FT_GlyphSlot
...
static void
  ft_glyphslot_done( FT_GlyphSlot
...
FT_Done_GlyphSlot( FT_GlyphSlot
...
static FT_Renderer
  ft_lookup_glyph_renderer( FT_GlyphSlot
...
static void
  ft_glyphslot_grid_fit_metrics( FT_GlyphSlot
...
horiBearingX = FT_PIX_FLOOR( metrics->horiBearingX );

metrics->

horiBearingY = FT_PIX_CEIL_LONG( metrics->horiBearingY );
...
vertBearingX = FT_PIX_FLOOR( metrics->vertBearingX );

metrics->

vertBearingY = FT_PIX_FLOOR( metrics->vertBearingY );

metrics->

width = SUB_LONG( right,
                  metrics->vertBearingX );

metrics->

height = SUB_LONG( bottom,
...
height = SUB_LONG( metrics->horiBearingY,
                   bottom );
}

metrics->

horiAdvance = FT_PIX_ROUND_LONG( metrics->horiAdvance );

metrics->

vertAdvance = FT_PIX_ROUND_LONG( metrics->vertAdvance );
}
...
linearHoriAdvance = FT_MulDiv( slot->linearHoriAdvance,
                               metrics->x_scale, 64 );

slot->

linearVertAdvance = FT_MulDiv( slot->linearVertAdvance,
                               metrics->y_scale, 64 );
}
...
/* transform advance */
FT_Vector_Transform( &slot

->advance, &internal->transform_matrix );
}
}
...
/**************************************************************************
   ...
   */
static FT_CharMap
  find_variant_selector_charmap( FT_Face
...
ft_open_face_internal( library, &args, face_index, aface,
...
static FT_Error
  ft_open_face_internal( FT_Library
...
FT_ERR_NEQ( error, Cannot_Open_Stream
...
max_advance_height = ( FT_Short ) face->height;
}
...
pathname = ( char* ) filepathname;

return

FT_Attach_Stream( face, &open
...
FT_THROW( Invalid_Driver_Handle );
...
size = ( FT_Size )( face->sizes_list.head->data );
}
...
ft_synthesize_vertical_metrics( FT_Glyph_Metrics
...
vertBearingX = metrics->horiBearingX - metrics->horiAdvance / 2;

metrics->

vertBearingY = ( advance - height ) / 2;
...
static void
  ft_recompute_scaled_metrics( FT_Face
...
ascender = FT_PIX_CEIL( FT_MulFix( face->ascender,
                                   metrics->y_scale ) );

metrics->

descender = FT_PIX_FLOOR( FT_MulFix( face->descender,
                                     metrics->y_scale ) );

metrics->

height = FT_PIX_ROUND( FT_MulFix( face->height,
                                  metrics->y_scale ) );

metrics->

max_advance = FT_PIX_ROUND( FT_MulFix( face->max_advance_width,
                                       metrics->x_scale ) );
...
x_ppem = ( FT_UShort )( ( bsize->x_ppem + 32 ) >> 6 );

metrics->

y_ppem = ( FT_UShort )( ( bsize->y_ppem + 32 ) >> 6 );
...
x_scale = FT_DivFix( bsize->x_ppem,
                     face->units_per_EM );

metrics->

y_scale = FT_DivFix( bsize->y_ppem,
                     face->units_per_EM );
...
x_scale = ( FT_Fixed ) req->width;

metrics->

y_scale = ( FT_Fixed ) req->height;
...
x_scale = FT_DivFix( scaled_w, w );

if( req->height )
{
metrics->

y_scale = FT_DivFix( scaled_h, h );
...
x_scale = metrics->y_scale = FT_DivFix( scaled_h, h );
...
x_ppem = ( FT_UShort )( ( scaled_w + 32 ) >> 6 );

metrics->

y_ppem = ( FT_UShort )( ( scaled_h + 32 ) >> 6 );
...
{
FT_Select_Metrics( face, (FT_ULong)
...
/* (if requested)                                       */
face->size->internal->autohint_metrics.
...
width = ( FT_Long )( pixel_width << 6 );

req.

height = ( FT_Long )( pixel_height << 6 );
...
x = FT_MulFix( akerning->x, face->size->metrics.x_scale );

akerning->

y = FT_MulFix( akerning->y, face->size->metrics.y_scale );
...
x = FT_PIX_ROUND( akerning->x );

akerning->

y = FT_PIX_ROUND( akerning->y );
...
face->charmaps[face->num_charmaps++] = (FT_CharMap)
...
load_table( face, tag, offset, buffer, length
...
table_info( face, table_index, tag, &offset, length
...
get_cmap_info( charmap, &cmap_info
...
static void
  ft_set_current_renderer( FT_Library
...
FT_Lookup_Renderer( library, format,
...
FT_Render_Glyph_Internal( FT_Library
...
FT_Render_Glyph_Internal( library, slot, render_mode
...
->clazz->module_name, clazz->module_name ) == 0 )
{
/* this installed module has the same name, compare their versions */
if( clazz->module_version <= module->clazz->module_version )
return

FT_THROW( Lower_Module_Version );
...
clazz = ( FT_Module_Class* ) clazz;

/* check whether the module is a renderer - this must be performed */
/* before the normal module initialization                         */
if(

FT_MODULE_IS_RENDERER( module )
...
clazz = ( FT_Driver_Class ) module->clazz;
}
...
library->modules[library->num_modules++] =
...
->clazz->module_name, module_name ) == 0 )
{
result = cur[0];
...
ft_module_get_service( FT_Module
...
ft_strcmp( module_name, driver_name[m]
...
library->debug_hooks[hook_index] =
