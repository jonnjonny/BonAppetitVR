...
FT_THROW( Invalid_Argument );  /* matrix can't be inverted */

matrix->

xy = -FT_DivFix( matrix->xy, delta );

matrix->

yx = -FT_DivFix( matrix->yx, delta );
...
FT_Vector_Transform_Scaled( FT_Vector
...
x = sx < 0 ? -( FT_Pos ) u : ( FT_Pos ) u;

vector->

y = sy < 0 ? -( FT_Pos ) v : ( FT_Pos ) v;
...
ADD_LONG( FT_ABS( in_x ), FT_ABS( out_y )


) <= 131071L &&

ADD_LONG( FT_ABS( in_y ), FT_ABS( out_x )
...
in_x, (FT_UInt32)out_y, &z1 );

ft_multo64( (FT_UInt32)

in_y, (FT_UInt32)out_x, &z2 );
