...
pitch = target->pitch < 0 ? -target_pitch : target_pitch;
}
break;
...
pitch* (FT_Int)( source->rows - 1 );

if( target->pitch < 0 )
t -= target->

pitch* (FT_Int)( target->rows - 1 );
...
ft_gray_for_premultiplied_srgb_bgra( ss );
...
pitch = ( int ) final_width * 4;

target->

num_grays = 256;

if( FT_LONG_MAX / target->
...
->buffer, target->pitch * (int)target->rows ) )
return
