...
x = ft_trig_downscale( v.x );

v.

y = ft_trig_downscale( v.y );
...
x = ( v.x + half - ( v.x < 0 ) ) >> shift;

vec->

y = ( v.y + half - ( v.y < 0 ) ) >> shift;
}
else
{
shift = -shift;
...
x = ( FT_Pos )( ( FT_ULong ) v.x << shift );

vec->

y = ( FT_Pos )( ( FT_ULong ) v.y << shift );
}
}
...
FT_Vector_From_Polar( FT_Vector
