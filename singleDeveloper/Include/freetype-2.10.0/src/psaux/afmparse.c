...
afm_parser_read_vals( AFM_Parser
...
f = PS_Conv_ToFixed( ( FT_Byte * * )( void * ) & str,
                     ( FT_Byte * )
...
i = PS_Conv_ToInt( ( FT_Byte * * )( void * ) & str,
                   ( FT_Byte * )
...
afm_parser_next_key( AFM_Parser
...
status = AFM_STREAM_STATUS_NORMAL;
...
static FT_Error
  afm_parser_read_int( AFM_Parser
...
type = AFM_VALUE_TYPE_INTEGER;

if(

afm_parser_read_vals( parser, &val,
...
static FT_Error
  afm_parse_track_kern( AFM_Parser
...
afm_parser_read_int( parser, &tmp
...
afm_parser_read_vals( parser, shared_vals,
...
degree = shared_vals[0].u.i;

tk->

min_ptsize = shared_vals[1].u.f;

tk->

min_kern = shared_vals[2].u.f;

tk->

max_ptsize = shared_vals[3].u.f;

tk->

max_kern = shared_vals[4].u.f;
...
static FT_Error
  afm_parse_kern_pairs( AFM_Parser
...
index1 = shared_vals[0].u.u;

kp->

index2 = shared_vals[1].u.u;
...
static FT_Error
  afm_parse_kern_data( AFM_Parser
...
static FT_Error
  afm_parser_skip_section( AFM_Parser
...
afm_parser_read_int( parser, &metrics_sets
...
xMin = shared_vals[0].u.f;

fi->FontBBox.

yMin = shared_vals[1].u.f;

fi->FontBBox.

xMax = shared_vals[2].u.f;

fi->FontBBox.

yMax = shared_vals[3].u.f;
