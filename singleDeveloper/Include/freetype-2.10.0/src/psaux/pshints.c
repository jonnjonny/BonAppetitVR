...
static CF2_Int
  cf2_getWindingMomentum( CF2_Fixed
...
cf2_hintmap_adjustHints( CF2_HintMap hintmap ) {
  ...
  for( i = 0; i < hintmap->count; i++ ) {
    FT_Bool
    isPair = cf2_hint_isPair( &hintmap->edge[i] );
    ...
    if( !cf2_hint_isLocked( &hintmap->edge[i] ) ) {
      ...
      CF2_Fixed
      fracDown = cf2_fixedFraction( hintmap->edge[i].dsCoord );
      CF2_Fixed
      fracUp = cf2_fixedFraction( hintmap->edge[j].dsCoord );
      ...
      /* smallest move up */
      CF2_Fixed
      moveUp = FT_MIN( downMoveUp, upMoveUp );
      /* smallest move down */
      CF2_Fixed
      moveDown = FT_MAX( downMoveDown, upMoveDown );
      ...
      CF2_Fixed
      downMinCounter = CF2_MIN_COUNTER;
      CF2_Fixed
      upMinCounter = CF2_MIN_COUNTER;
      FT_Bool
      saveEdge = FALSE;
...
cf2_hint_isLocked( &bottomHintEdge )
...
maskByte;      /* turn off the bit for this hint */
}
}
...
cf2_hint_isTop( &hintmap

->edge[i] ) )
stemhint->

maxDS = hintmap->edge[i].dsCoord;

else
stemhint->

minDS = hintmap->edge[i].dsCoord;
...
scaleX = font->innerTransform.a;

glyphpath->

scaleC = font->innerTransform.c;

glyphpath->

scaleY = font->innerTransform.d;
...
hintMask = hintMask;      /* ptr to current mask */
glyphpath->
...
darken = font->darkened; /* TODO: should we make copies? */
glyphpath->

xOffset = font->darkenX;

glyphpath->

yOffset = font->darkenY;
...
snapThreshold = cf2_doubleToFixed( 0.1 );
...
glyphpath )
{
cf2_arrstack_finalize( &glyphpath
...
x = glyphpath->start.x = x;

glyphpath->currentCS.

y = glyphpath->start.y = y;
...
cf2_hintmap_isValid( &glyphpath

->hintMap ) ||

cf2_hintmask_isNew( glyphpath

->hintMask )   )

cf2_hintmap_build( &glyphpath
...
x = ADD_INT32( glyphpath->currentCS.x, xOffset );

P0.

y = ADD_INT32( glyphpath->currentCS.y, yOffset );

P1.

x = ADD_INT32( x, xOffset );

P1.

y = ADD_INT32( y, yOffset );
...
moveIsPending = FALSE;  /* adjust state machine */
glyphpath->
...
offsetStart1 = P1;              /* record second point */
}
...
x = ADD_INT32( glyphpath->currentCS.x, xOffset1 );

P0.

y = ADD_INT32( glyphpath->currentCS.y, yOffset1 );

P1.

x = ADD_INT32( x1, xOffset1 );

P1.

y = ADD_INT32( y1, yOffset1 );

/* note: preserve angle of final segment by using offset3 at both ends */
P2.

x = ADD_INT32( x2, xOffset3 );

P2.

y = ADD_INT32( y2, yOffset3 );

P3.

x = ADD_INT32( x3, xOffset3 );

P3.

y = ADD_INT32( y3, yOffset3 );
