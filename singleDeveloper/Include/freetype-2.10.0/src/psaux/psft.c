...
cf2_decoder_parse_charstrings( PS_Decoder
...
cffload = ( FT_Service_CFFLoad ) decoder->cff->cffload;

/* initialize a client outline, to be shared by each glyph rendered */
cf2_outline_init( &font

->outline, font->memory, &font->error );
}
...
FT_ASSERT( charstring_base

+ charstring_len >= charstring_base );
...
end = charstring_base + charstring_len;
...
isCFF2 = ( ( TT_Face ) builder->face )->is_cff2;
}
font->
...
unitsPerEm = ( CF2_Int ) cf2_getUnitsPerEm( decoder );
...
return decoder->cff->top_font.font_dict.
...
cf2_getFamilyOtherBlues( PS_Decoder
...
cf2_getLanguageGroup( PS_Decoder
...
return decoder->current_subfont->private_dict.
...
cf2_initGlobalRegionBuffer( PS_Decoder
...
cf2_getSeacComponent( PS_Decoder
...
cf2_freeSeacComponent( PS_Decoder
...
cf2_getT1SeacComponent( PS_Decoder
...
if( !error )
{
FT_Byte* charstring_base = ( FT_Byte * )

glyph_data.

pointer;

FT_ULong charstring_len = ( FT_ULong )
...
cf2_freeT1SeacComponent( PS_Decoder
...
cf2_initLocalRegionBuffer( PS_Decoder
...
end = buf->start + decoder->locals_len[idx];
...
cf2_getDefaultWidthX( PS_Decoder
...
cf2_getNominalWidthX( PS_Decoder
...
ps_builder_close_contour( &decoder

->builder );

FT_GlyphLoader_Add( decoder
