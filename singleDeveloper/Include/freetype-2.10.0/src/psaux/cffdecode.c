...
cff_lookup_glyph_by_stdcharcode( CFF_Font
...
glyph_sid = cffload->get_standard_encoding( ( FT_UInt )
...
/* initialize builder */
cff_builder_init( &decoder

->builder, face, size, slot, hinting );
...
num_globals = cff->global_subrs_index.count;

decoder->

globals = cff->global_subrs;

decoder->

globals_bias = cff_compute_bias(
...
get_glyph_callback = get_callback;

decoder->

free_glyph_callback = free_callback;
}
...
cff_decoder_prepare( CFF_Decoder
...
hints_globals = ( void* ) internal->subfonts[fd_index];
}
}
...
nominal_width = sub->private_dict.nominal_width;
