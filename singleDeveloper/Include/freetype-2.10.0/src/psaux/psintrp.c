...
cf2_hintmask_setAll( CF2_HintMask
...
cf2_hintmask_setCounts( hintmask, bitCount
...
cf2_doFlex( CF2_Stack opStack,
  ...
  if( doConditionalLastRead ) {
    ...
    CF2_Fixed
    lastVal = cf2_stack_getReal( opStack, idx );
...
cf2_doBlend( const CFF_Blend blend,
  ...
  i, j;
  CF2_UInt
  numOperands = ( CF2_UInt )( numBlends * blend->lenBV );
  ...
  for( i = 0; i < numBlends; i++ ) {
    ...
    CF2_Fixed
    sum = cf2_stack_getReal( opStack, i + base );
...
cf2_interpT2CharString( CF2_Font
...
FT_ASSERT( known_othersubr_result_cnt
...
->isT1 ? decoder->builder.left_bearing->y
: 0 );
...
->isT1 ? decoder->builder.left_bearing->x
: 0 );
...
cf2_initGlobalRegionBuffer( decoder,
  subrNum,
  charstring
...
exit;  /* subroutine lookup or stream error */
}
break;
...
cf2_initLocalRegionBuffer( decoder,
  subrNum,
  charstring
...
FT_ASSERT( cf2_stack_count( opStack )
...
cf2_stack_pushInt( opStack, arg1
...
/* seac weirdness */
adx += decoder->builder.left_bearing->
...
/* prepare loader */
FT_GlyphLoader_Prepare( decoder
...
y = cf2_stack_popFixed( opStack );

builder->advance->

x = cf2_stack_popFixed( opStack );
...
{
divisor = ( CF2_F16Dot16 )

cf2_stack_popInt( opStack );

dividend = ( CF2_F16Dot16 )
...
ps_builder_check_points( &decoder
...
cf2_stack_setReal( opStack, values
...
known_othersubr_result_cnt = ( FT_Int )
...
x = ADD_INT32( builder->left_bearing->x,
               lsb_x );
...
FT_UInt32 byte1 = ( FT_UInt32 )

cf2_buf_readByte( charstring );

FT_UInt32 byte2 = ( FT_UInt32 )

cf2_buf_readByte( charstring );

FT_UInt32 byte3 = ( FT_UInt32 )

cf2_buf_readByte( charstring );

FT_UInt32 byte4 = ( FT_UInt32 )
...
cf2_stack_pushInt( opStack, (CF2_Int)
