...
FTC_MruNode_Prepend( (FTC_MruNode
...
FTC_MruNode_Remove( (FTC_MruNode
...
manager->cur_weight -= cache->clazz.

node_weight( node, cache
...
FTC_Cache_Init( FTC_Cache

cache )
{
return

ftc_cache_init( cache );
}
...
->buckets, FTC_HASH_INITIAL_SIZE * 2 );
return
...
FTC_Cache_RemoveFaceID( FTC_Cache
...
node_remove_faceid( node, face_id,
  cache, &list_changed
