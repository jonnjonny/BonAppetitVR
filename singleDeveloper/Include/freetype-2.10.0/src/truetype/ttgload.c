...
tt_get_metrics( TT_Loader loader,
  ...
  FT_UShort
  advance_width = 0, advance_height = 0;
...
TT_Forget_Glyph_Frame( TT_Loader
...
n_contours = FT_NEXT_SHORT( p );

loader->bbox.

xMin = FT_NEXT_SHORT( p );

loader->bbox.

yMin = FT_NEXT_SHORT( p );

loader->bbox.

xMax = FT_NEXT_SHORT( p );

loader->bbox.

yMax = FT_NEXT_SHORT( p );
...
arg1 = FT_NEXT_SHORT( p );

subglyph->

arg2 = FT_NEXT_SHORT( p );
}
else
{
subglyph->
...
TT_Init_Glyph_Loading( TT_Face

face )
{
face->

access_glyph_frame = TT_Access_Glyph_Frame;

face->

read_glyph_header = TT_Load_Glyph_Header;

face->

read_simple_glyph = TT_Load_Simple_Glyph;

face->

read_composite_glyph = TT_Load_Composite_Glyph;

face->

forget_glyph_frame = TT_Forget_Glyph_Frame;
}
...
tt_prepare_zone( TT_GlyphZone zone,
  ...
  zone->n_points = ( FT_UShort )
  load->outline.n_points -
  ( FT_UShort )
  start_point;
  ...
  zone->tags = ( FT_Byte * )
  load->outline.tags + start_point;
  zone->contours = ( FT_UShort * )
  load->outline.contours + start_contour;
  zone->first_point = ( FT_UShort )
  start_point;
}
...
zone->cur[zone->n_points - 4].

x =
FT_PIX_ROUND( zone->cur[zone->n_points - 4].x );

zone->cur[zone->n_points - 3].

x =
FT_PIX_ROUND( zone->cur[zone->n_points - 3].x );

zone->cur[zone->n_points - 2].

y =
FT_PIX_ROUND( zone->cur[zone->n_points - 2].y );

zone->cur[zone->n_points - 1].
...
pp1 = zone->cur[zone->n_points - 4];

loader->

pp2 = zone->cur[zone->n_points - 3];

loader->

pp3 = zone->cur[zone->n_points - 2];

loader->

pp4 = zone->cur[zone->n_points - 1];
...
outline->points[n_points + 1] = loader->

pp2;

outline->points[n_points + 2] = loader->

pp3;

outline->points[n_points + 3] = loader->
...
{
tt_prepare_zone( &loader

->zone, &gloader->current, 0, 0 );

FT_ARRAY_COPY( loader

->zone.orus, loader->zone.cur,
...
x = FT_MulFix( vec->x, x_scale );

vec->

y = FT_MulFix( vec->y, y_scale );
}
}
...
pp1 = outline->points[n_points - 4];

loader->

pp2 = outline->points[n_points - 3];
}
...
pp3 = outline->points[n_points - 2];

loader->

pp4 = outline->points[n_points - 1];
}
}
 ...
 */
static FT_Error
  TT_Process_Composite_Component( TT_Loader
...
points = gloader->base.outline.points +
         num_base_points;

current.

n_points = gloader->base.outline.n_points -
...
FT_Outline_Transform( &current, &subglyph
 ...
 */
static FT_Error
  TT_Process_Composite_Glyph( TT_Loader
...
outline->points[outline->n_points + 1] = loader->

pp2;

outline->points[outline->n_points + 2] = loader->

pp3;

outline->points[outline->n_points + 3] = loader->
...
x = loader->bbox.xMin - loader->left_bearing;
...
x = use_aw_2 ? loader->advance / 2 : 0;

loader->pp3.

y = loader->bbox.yMax + loader->top_bearing;
...
y = loader->pp3.y - loader->vadvance;
}
...
x = FT_MulFix( loader->pp1.x, x_scale );

loader->pp2.

x = FT_MulFix( loader->pp2.x, x_scale );

/* pp1.y and pp2.y are always zero */

loader->pp3.

x = FT_MulFix( loader->pp3.x, x_scale );

loader->pp3.

y = FT_MulFix( loader->pp3.y, y_scale );

loader->pp4.

x = FT_MulFix( loader->pp4.x, x_scale );

loader->pp4.

y = FT_MulFix( loader->pp4.y, y_scale );
}
...
data = FT_UINT_TO_POINTER( glyph_index );
...
static FT_Error
  compute_glyph_metrics( TT_Loader
...
linearHoriAdvance = loader->linear;

glyph->metrics.

horiBearingX = bbox.xMin;

glyph->metrics.

horiBearingY = bbox.yMax;

glyph->metrics.

horiAdvance = SUB_LONG( loader->pp2.x, loader->pp1.x );
...
width = SUB_LONG( bbox.xMax, bbox.xMin );

glyph->metrics.

height = SUB_LONG( bbox.yMax, bbox.yMin );
...
vertBearingX = SUB_LONG( glyph->metrics.horiBearingX,
...
static void
  tt_loader_done( TT_Loader

loader )
{
FT_List_Finalize( &loader
...
num_subglyphs = loader.gloader->base.num_subglyphs;

glyph->

subglyphs = loader.gloader->base.subglyphs;
}
else
{
glyph->
...
FT_Outline_Translate( &glyph

->outline, -loader.pp1.x, 0 );
}
