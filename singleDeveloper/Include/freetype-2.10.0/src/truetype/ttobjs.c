...
static FT_UInt32
  tt_synth_sfnt_checksum( FT_Stream
...
static FT_Bool
  tt_check_trickyness_sfnt_ids( TT_Face
...
static FT_Bool
  tt_check_single_notdef( FT_Face
...
TTAG_0xA5kbd && /* `Keyboard.dfont' (legacy Mac OS X)   */
  face

->format_tag != TTAG_0xA5lst )  /* `LastResort.dfont' (legacy Mac OS X) */
{
...
glyf_len &&FT_ERR_EQ( error, Table_Missing )
...
tt_size_init( FT_Size

ttsize )           /* TT_Size */
{
TT_Size size = ( TT_Size )
...
ascender = FT_PIX_ROUND(
  FT_MulFix( face->root.ascender,
             size_metrics->y_scale ) );

size_metrics->

descender = FT_PIX_ROUND(
...
x_scale = FT_DivFix( size_metrics->x_ppem << 6,
                     face->root.units_per_EM );

size_metrics->

y_scale = FT_DivFix( size_metrics->y_ppem << 6,
                     face->root.units_per_EM );

size_metrics->

max_advance = FT_PIX_ROUND(
...
scale = size_metrics->x_scale;

size->ttmetrics.

ppem = size_metrics->x_ppem;
...
y_ratio = FT_DivFix( size_metrics->y_ppem,
                     size_metrics->x_ppem );
}
else
{
...
scale = size_metrics->y_scale;

size->ttmetrics.

ppem = size_metrics->y_ppem;

size->ttmetrics.

x_ratio = FT_DivFix( size_metrics->x_ppem,
                     size_metrics->y_ppem );
...
tt_slot_init( FT_GlyphSlot

slot )
{
return

FT_GlyphLoader_CreateExtra( slot
