...
stack_size = sizeof( state->stack_0 );
...
max_bits = max_bits & LZW_BIT_MASK;

state->

block_mode = max_bits & LZW_BLOCK_MASK;

state->

max_free = ( FT_UInt )( ( 1UL << state->max_bits ) - 256 );
...
free_ent = ( state->block_mode ? LZW_FIRST
                               : LZW_CLEAR ) - 256;
...
free_bits = state->num_bits < state->max_bits
...
{
if( state->free_ent >= state->

prefix_size &&
  ft_lzwstate_prefix_grow( state )
