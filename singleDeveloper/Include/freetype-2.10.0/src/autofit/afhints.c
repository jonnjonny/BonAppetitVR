...
segments = axis->embedded.segments;

axis->

max_segments = AF_SEGMENTS_EMBEDDED;
}
}
...
new_max <old_max || new_max> big_max
...
af_axis_hints_new_edge( AF_AxisHints
...
edges = axis->embedded.edges;

axis->

max_edges = AF_EDGES_EMBEDDED;
}
}
...
af_glyph_hints_init( AF_GlyphHints
...
sizeof( *hints ) - sizeof( hints->embedded ) );
hints->
...
af_glyph_hints_done( AF_GlyphHints
...
af_glyph_hints_rescale( AF_GlyphHints
...
scaler_flags = metrics->scaler.flags;
}
...
af_glyph_hints_reload( AF_GlyphHints
...
contours = hints->embedded.contours;

hints->

max_contours = AF_CONTOURS_EMBEDDED;
}
}
...
FT_RENEW_ARRAY( hints

->contours, old_max, new_max ) )
goto
...
/*                                                                     */
hints->axis[AF_DIMENSION_HORZ].

major_dir = AF_DIR_UP;

hints->axis[AF_DIMENSION_VERT].

major_dir = AF_DIR_LEFT;
...
in_dir = ( FT_Char ) AF_DIR_NONE;

point->

out_dir = ( FT_Char ) AF_DIR_NONE;
...
ox = point->x = FT_MulFix( vec->x, x_scale ) + x_delta;

point->

oy = point->y = FT_MulFix( vec->y, y_scale ) + y_delta;

end->

fx = ( FT_Short ) outline->points[endpoint].x;

end->

fy = ( FT_Short ) outline->points[endpoint].y;
...
af_glyph_hints_save( AF_GlyphHints
...
af_glyph_hints_align_edge_points( AF_GlyphHints
...
af_glyph_hints_align_strong_points( AF_GlyphHints
...
scale = FT_DivFix( after->pos - before->pos,
...
af_glyph_hints_align_weak_points( AF_GlyphHints
...
af_iup_interp( first_point, first_touched
