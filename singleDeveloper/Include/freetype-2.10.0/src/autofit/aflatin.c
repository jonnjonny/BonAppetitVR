...
af_latin_metrics_init_widths( AF_LatinMetrics
...
af_glyph_hints_rescale( hints, (AF_StyleMetrics)
...
af_sort_and_quantize_widths( &num_widths, axis
...
af_latin_metrics_init_blues( AF_LatinMetrics metrics,
                             FT_Face face ) {
  FT_Pos
  flats[AF_BLUE_STRING_MAX_LEN];
  FT_Pos
  rounds[AF_BLUE_STRING_MAX_LEN];
  ...
  AF_StyleClass
  sc = metrics->root.style_class;

  AF_Blue_Stringset bss = sc->blue_stringset;
  const AF_Blue_StringRec* bs = &af_blue_stringsets[bss];

  FT_Pos
  flat_threshold = FLAT_THRESHOLD( metrics->units_per_em );
  ...
  for( ; bs->string != AF_BLUE_STRING_MAX; bs++ ) {
    ...
    while( *p ) {
      ...
      FT_Pos
      best_y_extremum;                      /* same as points.y */
      FT_Bool
      best_round = 0;
      ...
      for( i = 0; i < num_idx; i++ ) {
        ...
        if( best_point >= 0 ) {
          ...
          if( AF_LATIN_IS_LONG_BLUE( bs ) ) {
            ...
            FT_Pos
            length_threshold = metrics->units_per_em / 25;
            ...
            if( dist < length_threshold &&
              ...
              /* heuristic threshold value */
              FT_Pos
              height_threshold = metrics->units_per_em / 4;
...
af_latin_metrics_check_digits( AF_LatinMetrics
...
af_latin_metrics_init( AF_LatinMetrics
...
af_latin_metrics_scale( AF_LatinMetrics
...
af_latin_get_standard_widths( AF_LatinMetrics
...
af_latin_hints_compute_segments( AF_GlyphHints
...
pos = ( FT_Short )( ( min_pos + max_pos ) >> 1 );

segment->

delta = ( FT_Short )( ( max_pos - min_pos ) >> 1 );
...
min_coord = ( FT_Short ) min_coord;

segment->

max_coord = ( FT_Short ) max_coord;

segment->

height = segment->max_coord - segment->min_coord;
...
pos = ( FT_Short )( ( min_pos +
                      max_pos ) >> 1 );

prev_segment->

delta = ( FT_Short )( ( max_pos -
                        min_pos ) >> 1 );
...
pos = ( FT_Short )( ( prev_min_pos +
                      prev_max_pos ) >> 1 );

prev_segment->

delta = ( FT_Short )( ( prev_max_pos -
                        prev_min_pos ) >> 1 );
}
else
{
...
min_coord = ( FT_Short ) point->v;

segment->

max_coord = ( FT_Short ) point->v;
...
af_latin_hints_link_segments( AF_GlyphHints
...
af_latin_hints_compute_edges( AF_GlyphHints
...
af_latin_hints_detect_features( AF_GlyphHints
...
af_latin_hints_compute_blue_edges( AF_GlyphHints hints,
  ...
  AF_AxisHints
  axis = &hints->axis[AF_DIMENSION_VERT];
  ...
  for( ; edge < edge_limit; edge++ ) {
    ...
    FT_Bool
    best_blue_is_neutral = 0;
    FT_Pos
    best_dist;                 /* initial threshold */
    ...
    for( bb = 0; bb < latin->blue_count; bb++ ) {
      ...
      FT_Bool
      is_top_blue, is_neutral_blue, is_major_dir;
      ...
      if( is_top_blue ^ is_major_dir || is_neutral_blue ) {
        ...
        if( edge->flags & AF_EDGE_ROUND &&
            ...
            !is_neutral_blue ) {
          FT_Bool
          is_under_ref = FT_BOOL( edge->fpos < blue->ref.org );
...
af_latin_compute_stem_width( AF_GlyphHints hints,
                             ...
                             FT_UInt stem_flags ) {
  AF_LatinMetrics
  metrics = ( AF_LatinMetrics ) hints->metrics;
  AF_LatinAxis axis = &metrics->axis[dim];
  FT_Pos
  dist = width;
  FT_Int sign = 0;
...
af_latin_hint_edges( AF_GlyphHints hints,
  ...
  AF_StyleClass
  style_class = hints->metrics->style_class;
  AF_ScriptClass
  ...
  for( edge = edges; edge < edge_limit; edge++ ) {
    ...
    if( !anchor ) {
      ...
      FT_Pos
      org_len, org_center, cur_len;
      FT_Pos
      cur_pos1, error1, error2, u_off, d_off;
      ...
      FT_Pos
      org_pos, org_len, org_center, cur_len;
      FT_Pos
      cur_pos1, cur_pos2, delta1, delta2;
...
af_glyph_hints_align_edge_points( hints, (AF_Dimension)
