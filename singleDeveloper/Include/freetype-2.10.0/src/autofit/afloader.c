...
af_loader_embolden_glyph_in_slot( AF_Loader loader,
  ...
  FT_Bool
  size_changed = size_metrics->x_ppem !=
...
af_loader_load_glyph( AF_Loader
...
autohint_mode = FT_LOAD_TARGET_MODE( load_flags );

size_internal->

autohint_metrics = size->metrics;
...
x_scale = size_internal->autohint_metrics.x_scale;
...
y_scale = size_internal->autohint_metrics.y_scale;
...
style_metrics_scale( style_metrics, &scaler
...
transformed = slot_internal->glyph_transformed;
...
trans_matrix = slot_internal->glyph_matrix;

loader->

trans_delta = slot_internal->glyph_delta;
...
x = FT_MulFix( slot->metrics.horiAdvance,
...
x = FT_PIX_ROUND( pp1x_uh );

loader->pp2.

x = FT_PIX_ROUND( pp2x_uh );
...
x = FT_PIX_ROUND( pp1x + hints->xmin_delta );

loader->pp2.

x = FT_PIX_ROUND( pp2x + hints->xmax_delta );

slot->

lsb_delta = loader->pp1.x - pp1x;

slot->

rsb_delta = loader->pp2.x - pp2x;
}
}
...
x = FT_MulFix( vvector.x, style_metrics->scaler.x_scale );

vvector.

y = FT_MulFix( vvector.y, style_metrics->scaler.y_scale );

/* transform the hinted outline if needed */
if( loader->transformed )
{
FT_Outline_Transform( &gloader

->base.outline, &loader->trans_matrix );

FT_Vector_Transform( &vvector, &loader
...
FT_Outline_Translate( &gloader

->base.outline, -loader->pp1.x, 0 );

FT_Outline_Get_CBox( &gloader

->base.outline, &bbox );

bbox.

xMin = FT_PIX_FLOOR( bbox.xMin );

bbox.

yMin = FT_PIX_FLOOR( bbox.yMin );

bbox.

xMax = FT_PIX_CEIL( bbox.xMax );

bbox.

yMax = FT_PIX_CEIL( bbox.yMax );
...
vertBearingX = FT_PIX_FLOOR( bbox.xMin + vvector.x );

slot->metrics.

vertBearingY = FT_PIX_FLOOR( bbox.yMax + vvector.y );

/* for mono-width fonts (like Andale, Courier, etc.) we need */
/* to keep the original rounded advance width; ditto for     */
/* digits if all have the same advance width                 */
if( scaler.render_mode !=
...
af_face_globals_is_digit( loader
...
horiAdvance = loader->pp2.x - loader->pp1.x;
}

slot->metrics.

vertAdvance = FT_MulFix( slot->metrics.vertAdvance,
                         style_metrics->scaler.y_scale );

slot->metrics.

horiAdvance = FT_PIX_ROUND( slot->metrics.horiAdvance );

slot->metrics.

vertAdvance = FT_PIX_ROUND( slot->metrics.vertAdvance );
...
af_loader_compute_darkening( AF_Loader
