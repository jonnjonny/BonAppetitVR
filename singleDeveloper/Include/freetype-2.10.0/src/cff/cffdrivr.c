...
x = sfnt->get_kerning( face, left_glyph, right_glyph );
...
cff_glyph_load( FT_GlyphSlot

cffslot,      /* CFF_GlyphSlot */
...
CFF_GlyphSlot slot = ( CFF_GlyphSlot )
...
name_index( FT_FACE( face ), glyph_name
...
static FT_Int
  cff_ps_has_glyph_names( FT_Face

face )
{
return ( face->

face_flags &FT_FACE_FLAG_GLYPH_NAMES
...
version = cff_index_get_sid_string( cff,
                                    dict->version );

font_info->

notice = cff_index_get_sid_string( cff,
                                   dict->notice );

font_info->

full_name = cff_index_get_sid_string( cff,
                                      dict->full_name );
...
weight = cff_index_get_sid_string( cff,
                                   dict->weight );
...
is_fixed_pitch = dict->is_fixed_pitch;

font_info->

underline_position = ( FT_Short ) dict->underline_position;
...
static FT_Error
  cff_ps_get_font_extra( CFF_Face
...
start_fstype = ft_strstr( embedded_postscript,
                          "/FSType" )
...
s = start_fstype + sizeof( "/FSType" ) - 1;
...
cff_ps_get_font_info,    /* ps_get_font_info    */
...
get_ps_font_name( FT_FACE( face )

);
}

return (const char*)cff->
...
cff_get_cmap_info( FT_CharMap charmap,
  ...
  FT_Face
  face = FT_CMAP_FACE( cmap );
  FT_Library
  library = FT_FACE_LIBRARY( face );
...
cff_get_cmap_info    /* get_cmap_info */
)
...
registry = cff_index_get_sid_string( cff,
                                     dict->cid_registry );
...
ordering = cff_index_get_sid_string( cff,
                                     dict->cid_ordering );
...
static FT_Error
  cff_get_cid_from_glyph_index( CFF_Face
...
return sfnt ? sfnt->clazz->

get_interface( sfnt, module_interface
