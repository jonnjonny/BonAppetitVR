...
static FT_ULong
  cff_index_read_offset( CFF_Index
...
data_offset = idx->start + idx->hdr_size + size;
...
static FT_Error
  cff_index_load_offsets( CFF_Index
...
{
t[n] = new_bytes + next_offset +

extra;

if( next_offset != cur_offset )
{
FT_MEM_COPY( t[n - 1], org_bytes
...
cff_index_access_element( CFF_Index
...
cff_index_forget_element( CFF_Index
...
cff_index_get_sid_string( CFF_Font
...
static void
  CFF_Done_FD_Select( CFF_FDSelect
...
static FT_Error
  CFF_Load_FD_Select( CFF_FDSelect
...
FT_FRAME_EXTRACT( fdselect

->data_size, fdselect->data ) )
goto
...
cff_fd_select_get( CFF_FDSelect
...
static FT_Error
  cff_charset_compute_cids( CFF_Charset
...
FT_NEW_ARRAY( charset

->cids, (FT_ULong)max_cid + 1 ) )
goto
...
num_glyphs - 1; j >= 0; j-- )
charset->cids[charset->sids[j]] = (FT_UShort)
...
cff_charset_cid_to_gindex( CFF_Charset
...
static void
  cff_charset_free_cids( CFF_Charset
...
FT_STREAM_SEEK( charset

->offset ) ||

FT_READ_BYTE( charset
...
offset = offset;  /* record charset type */
...
FT_ARRAY_COPY( charset

->sids, cff_isoadobe_charset, num_glyphs );
...
->sids, cff_expert_charset, num_glyphs );
...
->sids, cff_expertsubset_charset, num_glyphs );
...
cff_blend_clear( CFF_SubFont

subFont )
{
subFont->

blend_top = subFont->blend_stack;
...
blend_top = subFont->blend_stack + subFont->blend_used;
...
blend_stack_old &&* p <blend_top_old
...
cff_parse_num( parser, &parser

->stack[delta++] ) * *weight++;

/* point parser stack to new value on blend_stack */
parser->stack[i + base] = subFont->
...
top = &parser->stack[base + numBlends];
...
cff_blend_build_vector( CFF_Blend
...
cff_blend_check_vector( CFF_Blend
      ...
      case 0: {
        ...
        for( j = 1; j <= count; j++ ) {
          ...
          if( j <num_glyphs ) {
            ...
            encoding->codes[glyph_code] = ( FT_UShort )
            j;
...
cff_load_private_dict( CFF_Font
...
usedBV = FALSE;  /* clear state */

if( !top->private_offset || !top->private_size )
goto

Exit2;       /* no private DICT, do nothing */
...
->cff2 ? CFF2_CODE_PRIVATE : CFF_CODE_PRIVATE,
priv,
font->library,
stackSize,
top->num_designs,
...
->base_offset + top->private_offset ) ||
...
initial_random_seed = -priv->initial_random_seed;
...
underline_position = -( 100L << 16 );

top->

underline_thickness = 50L << 16;
...
cid_ordering = 0xFFFFU;

top->

cid_font_name = 0xFFFFU;

/* set default stack size */
top->

maxstack = cff2 ? CFF2_DEFAULT_STACK : 48;
...
random = ( FT_UInt32 ) driver->random_seed;
...
random = ( FT_UInt32 ) face->root.internal->random_seed;
...
random = ( FT_UInt32 ) priv->initial_random_seed;
}
...
FT_STREAM_READ_FIELDS( cff_header_fields, font
...
data_offset = FT_STREAM_POS();

font->font_dict_index.

data_size = font->top_dict_length;
...
FT_SET_ERROR( cff_index_init( &font->name_index,
                              stream, 0, cff2 )
...
num_strings = string_index.count;
...
num_faces = font->name_index.count;
}
else
{
subfont_index = 0;
...
num_subfonts = fd_index.count;

if(

FT_NEW_ARRAY( sub, fd_index
...
num_glyphs = font->charstrings_index.count;
...
font_name = cff_index_get_name( font, subfont_index );
...
cff_encoding_done( &font

->encoding );

cff_charset_done( &font

->charset, font->stream );

cff_vstore_done( &font
...
CFF_Done_FD_Select( &font

->fd_select, font->stream );
