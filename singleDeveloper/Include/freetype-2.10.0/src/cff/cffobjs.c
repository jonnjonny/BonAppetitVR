...
static void
  cff_make_private_dict( CFF_SubFont
...
priv->standard_width[0]  = (FT_UShort)cpriv->

standard_width;

priv->standard_height[0] = (FT_UShort)cpriv->
...
force_bold = cpriv->force_bold;

priv->

language_group = cpriv->language_group;
...
module_data = internal;
}

size->

strike_index = 0xFFFFFFFFUL;
...
units_per_em = pure_cff ? 1000 : face->root.units_per_EM;
...
xx = FT_DivFix( matrix->xx, temp );

matrix->

yx = FT_DivFix( matrix->yx, temp );

matrix->

xy = FT_DivFix( matrix->xy, temp );

matrix->

yy = FT_DivFix( matrix->yy, temp );

offset->

x = FT_DivFix( offset->x, temp );

offset->

y = FT_DivFix( offset->y, temp );
}
...
FT_Matrix_Multiply_Scaled( &top

->font_matrix,
&sub->font_matrix,
scaling );

FT_Vector_Transform_Scaled( &sub
...
font_matrix = top->font_matrix;

sub->

font_offset = top->font_offset;

sub->

units_per_em = top->units_per_em;
}
...
num_glyphs = ( FT_Long )( cff->charset.max_cid + 1 );
...
xMin = dict->font_bbox.xMin >> 16;

cffface->bbox.

yMin = dict->font_bbox.yMin >> 16;

/* no `U' suffix here to 0xFFFF! */
cffface->bbox.

xMax = ( dict->font_bbox.xMax + 0xFFFF ) >> 16;

cffface->bbox.

yMax = ( dict->font_bbox.yMax + 0xFFFF ) >> 16;

cffface->

units_per_EM = ( FT_UShort )( dict->units_per_em );
...
descender = ( FT_Short )( cffface->bbox.yMin );

cffface->

height = ( FT_Short )( ( cffface->units_per_EM * 12 ) / 10 );

if( cffface->height <cffface->ascender - cffface->descender )
cffface->

height = ( FT_Short )( cffface->ascender -
                       cffface->descender );
...
family_name = cff_strcpy( memory, family_name );
}
...
family_name = cff_index_get_name(
  cff,
  ( FT_UInt )( face_index & 0xFFFF ) );

if( cffface->family_name )

remove_subset_prefix( cffface
...
family_name = cff_strcpy( memory, cid_font_name );
}
...
style_name = cff_strcpy( memory, ( char* ) "Regular" );
...
->top_font.font_dict.cid_registry != 0xFFFFU )
goto
...
encoding_id = TT_ADOBE_ID_EXPERT;
...
encoding_id = TT_ADOBE_ID_CUSTOM;
