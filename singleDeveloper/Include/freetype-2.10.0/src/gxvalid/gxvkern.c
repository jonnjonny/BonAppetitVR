...
static void
  gxv_kern_subtable_fmt0_pairs_validate( FT_Bytes
...
static void
  gxv_kern_subtable_fmt0_validate( FT_Bytes
...
static void
  gxv_kern_subtable_fmt1_valueTable_load( FT_Bytes
 ...
 */
static void
  gxv_kern_subtable_fmt1_subtable_setup( FT_UShort
...
static void
  gxv_kern_subtable_fmt1_validate( FT_Bytes
...
static void
  gxv_kern_subtable_fmt2_clstbl_validate( FT_Bytes
...
gxv_array_getlimits_ushort( p, p
...
static void
  gxv_kern_subtable_fmt2_validate( FT_Bytes
...
gxv_kern_subtable_fmt2_clstbl_validate( table

+ leftOffsetTable, limit,
GXV_KERN_CLS_L, gxvalid );

gxv_kern_subtable_fmt2_clstbl_validate( table

+ rightOffsetTable, limit,
...
static void
  gxv_kern_subtable_fmt3_validate( FT_Bytes
...
gxv_array_getlimits_byte( p, p

+ glyphCount, &min, &max, gxvalid );
p += gxvalid->
...
static FT_Bool
  gxv_kern_coverage_new_apple_validate( FT_UShort
...
static FT_Bool
  gxv_kern_coverage_classic_apple_validate( FT_UShort
...
static FT_Bool
  gxv_kern_coverage_classic_microsoft_validate( FT_UShort
...
static GXV_kern_Dialect
  gxv_kern_coverage_validate( FT_UShort
...
gxv_kern_coverage_new_apple_validate( coverage,
  format,
  gxvalid
...
gxv_kern_coverage_classic_apple_validate( coverage,
  format,
  gxvalid
...
gxv_kern_coverage_classic_microsoft_validate( coverage,
  format,
  gxvalid
...
static void
  gxv_kern_subtable_validate( FT_Bytes
...
FT_NEXT_USHORT( p ); /* Apple: length_hi MS: version */
u16[1]   =

FT_NEXT_USHORT( p ); /* Apple: length_lo MS: length */
coverage = FT_NEXT_USHORT( p );

switch(

gxv_kern_coverage_validate( coverage, &format, gxvalid
...
gxv_kern_subtable_fmt0_validate( table, table

+ length, gxvalid );
else if( format == 1 )

gxv_kern_subtable_fmt1_validate( table, table

+ length, gxvalid );
else if( format == 2 )

gxv_kern_subtable_fmt2_validate( table, table

+ length, gxvalid );
else if( format == 3 )

gxv_kern_subtable_fmt3_validate( table, table
...
static void
  gxv_kern_validate_generic( FT_Bytes
...
GXV_INIT;
KERN_DIALECT( gxvalid ) =

dialect_request;

GXV_LIMIT_CHECK( 2 );
GXV_KERN_DATA( version ) = (GXV_kern_Version)
...
ftvalid )
{
gxv_kern_validate_generic( table, face,

0, KERN_DIALECT_ANY, ftvalid );
}
...
gxv_kern_validate_classic( FT_Bytes
