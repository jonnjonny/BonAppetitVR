...
static void
  gxv_just_check_max_gid( FT_UShort
...
static void
  gxv_just_wdp_entry_validate( FT_Bytes
...
static void
  gxv_just_wdc_entry_validate( FT_Bytes
...
static void
  gxv_just_widthDeltaClusters_validate( FT_Bytes
...
static void
  gxv_just_actSubrecord_type0_validate( FT_Bytes
...
static void
  gxv_just_actSubrecord_type1_validate( FT_Bytes
...
gxv_just_check_max_gid( addGlyph,
...
static void
  gxv_just_actSubrecord_type2_validate( FT_Bytes
...
gxv_just_check_max_gid( substGlyph,
...
static void
  gxv_just_actSubrecord_type4_validate( FT_Bytes
...
static void
  gxv_just_actSubrecord_type5_validate( FT_Bytes
...
static void
  gxv_just_actSubrecord_validate( FT_Bytes
...
else if( actionType == 3 );                         /* Stretch glyph action: no actionData */
...
static void
  gxv_just_pcActionRecord_validate( FT_Bytes
...
static void
  gxv_just_pcTable_LookupValue_entry_validate( FT_UShort
...
GXV_JUST_DATA( pc_offset_max ) = value_p->

u;

if( value_p->u < GXV_JUST_DATA( pc_offset_max ) )
GXV_JUST_DATA( pc_offset_min ) = value_p->
...
static void
  gxv_just_pcLookupTable_validate( FT_Bytes
...
static void
  gxv_just_postcompTable_validate( FT_Bytes
...
static void
  gxv_just_justClassTable_validate( FT_Bytes
...
optdata_load_func = NULL;

gxvalid->statetable.

subtable_setup_func = NULL;

gxvalid->statetable.

entry_glyphoffset_fmt = GXV_GLYPHOFFSET_NONE;
...
gxv_StateTable_validate( p, table
...
static void
  gxv_just_wdcTable_LookupValue_validate( FT_UShort
...
GXV_JUST_DATA( wdc_offset_max ) = value_p->

u;

if( value_p->u < GXV_JUST_DATA( wdc_offset_min ) )
GXV_JUST_DATA( wdc_offset_min ) = value_p->
...
static void
  gxv_just_justData_lookuptable_validate( FT_Bytes
...
lookupval_func = gxv_just_wdcTable_LookupValue_validate;
 ...
 */
static void
  gxv_just_justData_validate( FT_Bytes
...
gxv_odtect_add_range( p, gxvalid
...
->root->base + wdcTableOffset, limit, gxvalid );

gxv_odtect_add_range( gxvalid
...
if( pcTableOffset )
{
gxv_just_postcompTable_validate( gxvalid
...
->root->base + justClassTableOffset, limit, gxvalid );
...
+ horizOffset, limit, gxvalid );

gxv_odtect_add_range( table

+ horizOffset, gxvalid->subtable_length,
...
+ vertOffset, limit, gxvalid );

gxv_odtect_add_range( table

+ vertOffset, gxvalid->subtable_length,
"vertJustData", odtect );
}
