...
gxv_mort_featurearray_validate( FT_Bytes
...
featureType = FT_NEXT_USHORT( p );

f.

featureSetting = FT_NEXT_USHORT( p );

f.

enableFlags = FT_NEXT_ULONG( p );

f.

disableFlags = FT_NEXT_ULONG( p );
...
gxv_mort_coverage_validate( FT_UShort
...
static void
  gxv_mort_subtables_validate( FT_Bytes
...
static void
  gxv_mort_chain_validate( FT_Bytes
...
gxv_mort_featurearray_validate( p, table
...
gxv_mort_subtables_validate( p, table

+ chainLength, nSubtables, gxvalid );
gxvalid->

subtable_length = chainLength;
