...
T1_GlyphSlot_Done( FT_GlyphSlot
...
T1_GlyphSlot_Init( FT_GlyphSlot
...
->len_buildchar == 0 ) == ( face->buildchar == NULL ) );
...
T1_Done_Metrics( memory, (AFM_FontInfo)
...
pshinter = FT_Get_Module_Interface( FT_FACE_LIBRARY( face ),
                                    "pshinter" );
...
xMin = type1->font_bbox.xMin >> 16;

root->bbox.

yMin = type1->font_bbox.yMin >> 16;

/* no `U' suffix here to 0xFFFF! */
root->bbox.

xMax = ( type1->font_bbox.xMax + 0xFFFF ) >> 16;

root->bbox.

yMax = ( type1->font_bbox.yMax + 0xFFFF ) >> 16;
...
ascender = ( FT_Short )( root->bbox.yMax );

root->

descender = ( FT_Short )( root->bbox.yMin );

root->

height = ( FT_Short )( ( root->units_per_EM * 12 ) / 10 );

if( root->height <root->ascender - root->descender )
root->

height = ( FT_Short )( root->ascender - root->descender );
...
max_advance_width = ( FT_Short ) FIXED_TO_INT( max_advance );
...
max_advance_height = root->height;
