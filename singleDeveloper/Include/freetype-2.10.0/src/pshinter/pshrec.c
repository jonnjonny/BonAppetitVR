...
ps_mask_table_merge_all( PS_Mask_Table table,
  ...
  for( index1 = ( FT_Int )table->num_masks - 1; index1 > 0;
  index1-- )
  {
  ...
  if( !hints->error ) {
    ...
    if( hints->hint_type == PS_HINT_TYPE_1 ) {
      ...
      for( count = 0; count < 3; count++, stems += 2 ) {
        error = ps_dimension_add_t1stem( dim,
                                         ( FT_Int )
        FIXED_TO_INT( stems[0] ),
          ( FT_Int )
        FIXED_TO_INT( stems[1] ),
          memory, &idx[count] );
        if( error )
          goto Fail;
      }
...
t1_hints_funcs_init( T1_Hints_FuncsRec
...
open = ( T1_Hints_OpenFunc ) t1_hints_open;

funcs->

close = ( T1_Hints_CloseFunc ) ps_hints_close;

funcs->

stem = ( T1_Hints_SetStemFunc ) t1_hints_stem;

funcs->

stem3 = ( T1_Hints_SetStem3Func ) ps_hints_t1stem3;

funcs->

reset = ( T1_Hints_ResetFunc ) ps_hints_t1reset;

funcs->

apply = ( T1_Hints_ApplyFunc ) ps_hints_apply;
}
...
t2_hints_funcs_init( T2_Hints_FuncsRec
...
open = ( T2_Hints_OpenFunc ) t2_hints_open;

funcs->

close = ( T2_Hints_CloseFunc ) ps_hints_close;

funcs->

stems = ( T2_Hints_StemsFunc ) t2_hints_stems;

funcs->

hintmask = ( T2_Hints_MaskFunc ) ps_hints_t2mask;

funcs->

counter = ( T2_Hints_CounterFunc ) ps_hints_t2counter;

funcs->

apply = ( T2_Hints_ApplyFunc ) ps_hints_apply;
}
