...
static FT_Fixed
  psh_hint_snap_stem_side_delta( FT_Fixed
...
psh_hint_align( PSH_Hint hint,
  ...
  FT_Fixed
  scale = dim->scale_mult;
  FT_Fixed
  delta = dim->scale_delta;
...
psh_glyph_init( PSH_Glyph glyph,
...

  {
    ...
    for( n = 0; n < glyph->num_points; n++, point++ ) {
      FT_Int
      n_prev = ( FT_Int )( point->prev - points );
      FT_Int
      n_next = ( FT_Int )( point->next - points );
...
psh_glyph_find_strong_points( PSH_Glyph glyph,
  ...
  FT_UInt first = 0;
  FT_Int
  major_dir = ( dimension == 0 ) ? PSH_DIR_VERTICAL
                                 : PSH_DIR_HORIZONTAL;
  ...
  threshold = ( FT_Int )
  FT_DivFix( PSH_STRONG_THRESHOLD, scale );
...
ps_hints_apply( PS_Hints ps_hints,
   ...
   */
  {
    ...
    FT_Fixed
    x_scale = dim_x->scale_mult;
    FT_Fixed
    y_scale = dim_y->scale_mult;
