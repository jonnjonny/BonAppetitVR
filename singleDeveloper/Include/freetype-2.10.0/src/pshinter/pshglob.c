...
static void
  psh_globals_scale_widths( PSH_Globals
...
cur = FT_MulFix( width->org, scale );

width->

fit = FT_PIX_ROUND( width->cur );
...
static void
  psh_globals_destroy( PSH_Globals
...
globals->dimension[0].stdw.

count = 0;

globals->dimension[1].stdw.

count = 0;

globals->blues.normal_top.

count = 0;

globals->blues.normal_bottom.

count = 0;

globals->blues.family_top.

count = 0;

globals->blues.family_bottom.
...
psh_globals_set_scale( PSH_Globals
...
psh_globals_scale_widths( globals,
...
psh_blues_scale_zones( &globals

->blues, y_scale, y_delta );
}
}
...
psh_globals_funcs_init( PSH_Globals_FuncsRec
