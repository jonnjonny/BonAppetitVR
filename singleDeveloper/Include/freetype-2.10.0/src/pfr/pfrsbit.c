...
n++ )
{
if( strike->x_ppm == (FT_UInt)size->root.metrics.

x_ppem &&
  strike

->y_ppm == (FT_UInt)size->root.metrics.y_ppem )
goto
...
->bct_offset + strike->bct_offset ) ||

FT_FRAME_ENTER( char_len
...
->header.gps_section_offset + gps_offset ) ||
...
width = ( FT_Pos ) xsize << 6;

glyph->root.metrics.

height = ( FT_Pos ) ysize << 6;

glyph->root.metrics.

horiBearingX = xpos * 64;

glyph->root.metrics.

horiBearingY = ypos * 64;

glyph->root.metrics.

horiAdvance = FT_PIX_ROUND( ( advance >> 2 ) );

glyph->root.metrics.

vertBearingX = -glyph->root.metrics.width >> 1;
...
vertAdvance = size->root.metrics.height;
...
bitmap_top = ( FT_Int )( ypos + ( FT_Long )
