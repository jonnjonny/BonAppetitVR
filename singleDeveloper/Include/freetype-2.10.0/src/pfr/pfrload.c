...
pfr_extra_items_parse( pp, limit, NULL, NULL
...
stroke_thickness = ( flags & PFR_LOG_2BYTE_STROKE )
                   ? PFR_NEXT_SHORT( p )
                   : PFR_NEXT_BYTE( p );
...
bold_thickness = ( flags & PFR_LOG_2BYTE_BOLD )
                 ? PFR_NEXT_SHORT( p )
                 : PFR_NEXT_BYTE( p );
}
...
phys_size = PFR_NEXT_USHORT( p );

log_font->

phys_offset = PFR_NEXT_ULONG( p );

if( size_increment )
{
PFR_CHECK( 1 );
log_font->phys_size += (FT_UInt32)
...
pfr_extra_item_load_bitmap_info( FT_Byte
...
x_ppm = ( flags0 & PFR_STRIKE_2BYTE_XPPM )
        ? PFR_NEXT_USHORT( p )
        : PFR_NEXT_BYTE( p );

strike->

y_ppm = ( flags0 & PFR_STRIKE_2BYTE_YPPM )
        ? PFR_NEXT_USHORT( p )
        : PFR_NEXT_BYTE( p );
...
bct_size = ( flags0 & PFR_STRIKE_3BYTE_SIZE )
           ? PFR_NEXT_ULONG( p )
           : PFR_NEXT_USHORT( p );

strike->

bct_offset = ( flags0 & PFR_STRIKE_3BYTE_OFFSET )
             ? PFR_NEXT_ULONG( p )
             : PFR_NEXT_USHORT( p );

strike->

num_bitmaps = ( flags0 & PFR_STRIKE_2BYTE_COUNT )
              ? PFR_NEXT_USHORT( p )
              : PFR_NEXT_BYTE( p );
}
...
pfr_extra_item_load_font_id( FT_Byte
...
pfr_extra_item_load_stem_snaps( FT_Byte
...
pfr_extra_item_load_kerning_pairs( FT_Byte
...
pair1 = PFR_KERN_INDEX( char1, char2 );
...
pair2 = PFR_KERN_INDEX( char1, char2 );
}
else
{
q = p;
...
kern_items_tail = item;

phy_font->

kern_items_tail = &item->next;

phy_font->num_kern_pairs  += item->
...
kern_items_tail = &phy_font->kern_items;
...
font_ref_number = PFR_NEXT_USHORT( p );

phy_font->

outline_resolution = PFR_NEXT_USHORT( p );

phy_font->

metrics_resolution = PFR_NEXT_USHORT( p );

phy_font->bbox.

xMin = PFR_NEXT_SHORT( p );

phy_font->bbox.

yMin = PFR_NEXT_SHORT( p );

phy_font->bbox.

xMax = PFR_NEXT_SHORT( p );

phy_font->bbox.

yMax = PFR_NEXT_SHORT( p );

phy_font->

flags = flags = PFR_NEXT_BYTE( p );
...
standard_advance = PFR_NEXT_SHORT( p );
}
...
ascent = PFR_NEXT_SHORT( q );

phy_font->

descent = PFR_NEXT_SHORT( q );

phy_font->

leading = PFR_NEXT_SHORT( q );
...
num_blue_values = count = PFR_NEXT_BYTE( p );

PFR_CHECK( count * 2 );

if(

FT_NEW_ARRAY( phy_font
...
blue_fuzz = PFR_NEXT_BYTE( p );

phy_font->

blue_scale = PFR_NEXT_BYTE( p );

phy_font->vertical.

standard = PFR_NEXT_USHORT( p );
...
num_chars = count = PFR_NEXT_USHORT( p );

phy_font->

chars_offset = offset + ( FT_Offset )( p - stream->cursor );
...
gps_size = ( flags & PFR_PHY_2BYTE_GPS_SIZE )
           ? PFR_NEXT_USHORT( p )
           : PFR_NEXT_BYTE( p );

cur->

gps_offset = ( flags & PFR_PHY_3BYTE_GPS_OFFSET )
             ? PFR_NEXT_ULONG( p )
             : PFR_NEXT_USHORT( p );
}
}
