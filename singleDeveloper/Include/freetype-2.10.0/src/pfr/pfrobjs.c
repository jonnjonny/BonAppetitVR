...
family_name = phy_font->family_name;

if( !pfrface->family_name )
pfrface->

family_name = phy_font->font_id;
...
style_name = phy_font->style_name;
...
units_per_EM = ( FT_UShort ) phy_font->outline_resolution;

pfrface->

ascender = ( FT_Short ) phy_font->bbox.yMax;

pfrface->

descender = ( FT_Short ) phy_font->bbox.yMin;

pfrface->

height = ( FT_Short )( ( pfrface->units_per_EM * 12 ) / 10 );

if( pfrface->height <pfrface->ascender - pfrface->descender )
pfrface->

height = ( FT_Short )( pfrface->ascender - pfrface->descender );
...
FT_NEW_ARRAY( pfrface

->available_sizes, count ) )
goto
...
height = ( FT_Short ) strike->y_ppm;

size->

width = ( FT_Short ) strike->x_ppm;

size->

size = ( FT_Pos )( strike->y_ppm << 6 );

size->

x_ppem = ( FT_Pos )( strike->x_ppm << 6 );

size->

y_ppem = ( FT_Pos )( strike->y_ppm << 6 );
}
pfrface->
...
max_advance_width = ( FT_Short ) phy_font->standard_advance;
...
max_advance_width = ( FT_Short ) max;
}

pfrface->

max_advance_height = pfrface->height;

pfrface->

underline_position = ( FT_Short )( -pfrface->units_per_EM / 10 );

pfrface->

underline_thickness = ( FT_Short )( pfrface->units_per_EM / 30 );
...
linearHoriAdvance = metrics->horiAdvance;
...
horiBearingY = cbox.yMax - metrics->height;
}
