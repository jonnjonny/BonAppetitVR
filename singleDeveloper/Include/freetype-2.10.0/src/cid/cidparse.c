...
offset = FT_STREAM_POS();; offset += 256 )
{
FT_ULong stream_len;
...
+ read_offset + read_len - STARTDATA_LEN,
STARTDATA_LEN );
...
FT_FRAME_EXTRACT( ps_len, parser
...
postscript_len = ps_len;

parser->root.

base = parser->postscript;

parser->root.

cursor = parser->postscript;

parser->root.

limit = parser->root.cursor + ps_len;
