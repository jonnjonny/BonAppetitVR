...
cid_parse_font_matrix( CID_Face
...
parse_expansion_factor( CID_Face
...
expansion_factor = cid_parser_to_fixed( parser, 0 );

dict->private_dict.

expansion_factor = dict->expansion_factor;
...
FT_RENEW_ARRAY( offsets, max_offsets, new_max
...
->data_offset + dict->subrmap_offset )     ||

FT_FRAME_ENTER( ( num_subrs

+ 1 ) * (FT_UInt)dict->sd_bytes ) )
goto
...
subr->code[count] = subr->code[count - 1] +
...
binary_length = face->root.stream->size -
                parser->data_offset;
}
...
cid_stream = *face->root.stream;

cid->

data_offset = loader.parser.data_offset;
}
...
->num_subrs > ( binary_length - dict->subrmap_offset ) /
