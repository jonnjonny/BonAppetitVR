...
num_tables = valid_entries;

face->

format_tag = sfnt.format_tag;

if(

FT_QNEW_ARRAY( face

->dir_tables, face->num_tables ) )
goto
...
Length = ( stream->size - entry.Offset ) & ~3U;
...
FT_STREAM_READ_AT( offset, buffer, size
 ...
 */
static FT_Error
  tt_face_load_generic_header( TT_Face
...
FT_STREAM_READ_FIELDS( header_fields, header
...
tt_face_load_generic_header( face, stream, TTAG_head
...
FT_STREAM_READ_FIELDS( maxp_fields, maxProfile
...
FT_STREAM_READ_FIELDS( maxp_fields_extra, maxProfile
...
FT_STREAM_READ_FIELDS( name_table_fields, table
...
->langTags, table->numLangTagRecords ) ||
...
FT_STREAM_READ_FIELDS( langTag_record_fields, entry

);

/* check that the langTag string is within the table */
entry->stringOffset += table_pos + table->

storageOffset;

if( entry->

stringOffset <storage_start ||
...
FT_STREAM_READ_FIELDS( name_record_fields, entry
...
num_names = ( FT_UShort ) table->numNameRecords;
...
FT_FRAME_EXTRACT( face

->cmap_size, face->cmap_table ) )
face->
...
FT_STREAM_READ_FIELDS( os2_fields, os2
...
FT_STREAM_READ_FIELDS( os2_fields_extra1, os2
...
FT_STREAM_READ_FIELDS( os2_fields_extra2, os2
...
FT_STREAM_READ_FIELDS( os2_fields_extra5, os2
...
FT_STREAM_READ_FIELDS( post_fields, post
...
FT_STREAM_READ_FIELDS( pclt_fields, pclt
...
->gasp.gaspRanges, num_ranges ) ||

FT_FRAME_ENTER( num_ranges
...
maxPPEM = FT_GET_USHORT();

gaspranges[j].

gaspFlag = FT_GET_USHORT();
