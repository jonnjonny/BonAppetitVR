...
platform_id = TT_NEXT_USHORT( p );

charmap.

encoding_id = TT_NEXT_USHORT( p );
...
num_glyphs = ( FT_UInt ) face->max_profile.numGlyphs;

if(

ft_setjmp( FT_VALIDATOR( &valid )
...
FT_THROW( Invalid_CharMap_Format );
}
