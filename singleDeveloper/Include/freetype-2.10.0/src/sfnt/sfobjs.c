...
compare_offsets( const void* a,
  ...
  FT_ULong
  offset1 = table1->Offset;
  FT_ULong
  offset2 = table2->Offset;
...
FT_STREAM_READ_FIELDS( woff_header_fields, &woff
...
FT_NEW_ARRAY( tables, woff

.num_tables )  ||

FT_NEW_ARRAY( indices, woff
...
CompLength = FT_GET_ULONG();

table->

OrigLength = FT_GET_ULONG();
...
FT_Stream_OpenMemory( sfnt_stream, sfnt, woff
...
face_flags &FT_FACE_FLAG_EXTERNAL_STREAM
...
FT_STREAM_READ_FIELDS( ttc_header_fields, &face
...
->ttc_header.offsets, face->ttc_header.count ) )
return
...
goto_table = sfnt->goto_table;
}

FT_FACE_FIND_GLOBAL_SERVICE( face, face

->psnames, POSTSCRIPT_CMAPS );
...
num_faces = face->ttc_header.count;

face->root.

face_index = face_instance_index;
...
error &&FT_ERR_NEQ( error, Table_Missing )
...
FT_FACE_FLAG_COLOR;      /* color glyphs */

if( has_outline == TRUE )
flags |=

FT_FACE_FLAG_SCALABLE;   /* scalable outlines */
...
units_per_EM = face->header.Units_Per_EM;
...
height = root->ascender - root->descender +
...
ascender = ( FT_Short ) face->os2.usWinAscent;

root->

descender = -( FT_Short ) face->os2.usWinDescent;
...
underline_position = face->postscript.underlinePosition -
                     face->postscript.underlineThickness / 2;

root->

underline_thickness = face->postscript.underlineThickness;
}
...
->vertical.long_metrics  );

FT_FREE( face

->vertical.short_metrics );
face->
...
->palette_data.palette_name_ids );

FT_FREE( face

->palette_data.palette_flags );

FT_FREE( face

->palette_data.palette_entry_name_ids );
