...
> cur_size ) ? cur_size : new_size );
#ifdef __amigaos4__
FreeVecPooled ( memory->user, block );
#else

Free_VecPooled ( memory
...
iobuf_end = 0; /* trigger a buffer refill */
}
...
->iobuf[offset - sysfile->iobuf_start], buffer, count );
...
iobuf_start = sysfile->iobuf_end - bufsize;

CopyMem( &buffer[read_bytes - bufsize], sysfile
...
file = Open( ( STRPTR ) filepathname, MODE_OLDFILE );
...
read = ft_amiga_stream_io;

stream->

close = ft_amiga_stream_close;
...
#else
  memory = ( FT_Memory )
  AllocVec( sizeof( *memory ), MEMF_PUBLIC );
#endif
  if( memory ) {
#ifdef __amigaos4__
